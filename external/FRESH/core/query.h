...
template <typename Point>
void query_continuous_exact(const std::vector<Curve<Point>>& dataset, std::vector<Curve<Point>>& queries, double range,
                            const std::vector<double>& simplification_epsilons, Xorshift1024star& rnd,
...
template <typename Point>
void query_continuous_basic(const std::vector<Curve<Point>>& dataset, std::vector<Curve<Point>>& queries, double range,
                            Xorshift1024star& rnd, std::function<void(size_t, size_t)> positive_callback) {
    START_TIMER(algorithm);
    const size_t n = dataset.size();
        ...
        for (size_t curve_idx = 0; curve_idx < n; curve_idx++) {
            ...
            if (babr::get_frechet_distance_upper_bound(query, candidate) <= range) {
                ...
                if (query.id == candidate.id)
                    printf("WARNING, ignoring match of %lu with itself (negfilter)", query.id);
                continue;
...
template <typename Point>
void query_hash(const std::vector<Curve<Point>>& dataset, std::vector<Curve<Point>>& queries, double range,
                const std::vector<double>& simplification_epsilons, size_t k, size_t L, double resolution_factor,
                Xorshift1024star& rnd, std::function<void(size_t, size_t)> positive_callback) {
    START_TIMER(algorithm);
...
#pragma omp parallel for
    for (size_t query_idx = 0; query_idx < queries.size(); query_idx++) {
        ...
        lsh_table.foreach_collision(query.points, tl_visited, [&](size_t collision_idx) {
            if (continuous_frechet_distance_predicate(query, dataset[collision_idx], range, simplification_epsilons)) {
...
template <typename Point>
void query_hash_no_eval(const std::vector<Curve<Point>>& dataset, std::vector<Curve<Point>>& queries, double range,
                        const std::vector<double>& simplification_epsilons, size_t k, size_t L,
                        double resolution_factor, Xorshift1024star& rnd,
