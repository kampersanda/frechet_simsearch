...
void densify(Curve<Point2D>& curve, double max_gap, std::vector<Point2D>& scratch_curve) {
    ...
    for (size_t orig_idx = 0; orig_idx < n - 1; orig_idx++) {
        ...
        const double d = euclidean(scratch_curve[orig_idx], scratch_curve[orig_idx + 1]);
        if (d > max_gap) {
            ...
            const double diff_x = scratch_curve[orig_idx + 1].x - scratch_curve[orig_idx].x;
            const double diff_y = scratch_curve[orig_idx + 1].y - scratch_curve[orig_idx].y;
