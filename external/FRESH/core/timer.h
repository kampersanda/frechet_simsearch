...
#define GET_TIMER_MS(___name) ___name.elapsed().wall / 1000000.0
...
#define GET_WALL_SUM(name_str) TimeRegistry::getInstance().get_wall_sum(name_str)
...
#define EXPERIMENT_RECORD_PROFILE()                                                           \
    for (const auto& wall : TimeRegistry::getInstance().get_walls()) {                        \
        ...
        for (const auto& time : wall.second) {                                                \
            EXPERIMENT_APPEND("profile", {{"timer", timer}, {"measure", i}, {"time", time}}); \
            i++;                                                                              \
        }                                                                                     \
    }
