...
std::string make_result_path(std::string result_dir, std::string base_path, std::string query_path,
    ...
    auto result_path = tfm::format("%s/hmsearch_perf-%s-%s-%g-%s-%s.mpk",  //
...
int main_template(const cmdline::parser& p) {
    ...
    if (repeatations > 64) {
        errorfln("repeatations > 64 is not supported");
        return 1;
    }
    ...
    EXPERIMENT_TAG("algorithm", std::string("HmSearch"));
    ...
    std::unique_ptr<HmsearchIndex<Point, Length>> index;
    ...
    for (uint32_t hamming_range = min_range; hamming_range <= max_range; hamming_range += step) {
        ...
        const uint32_t buckets = hmsearch::hm_index::get_proper_buckets(hamming_range);

        /**
         *  Build index
         */
        if (!index.get() || buckets != cfg.buckets) {
            START_TIMER(preprocessing);
            cfg.buckets = buckets;
            index = std::make_unique<HmsearchIndex<Point, Length>>(base_curves, cfg);
            STOP_TIMER_V(preprocessing);

            constr_sec = GET_TIMER_MS(preprocessing) / 1000.0;
        ...
        EXPERIMENT_APPEND("profile", {{"hamming_range", size_t(hamming_range)},  //
                                      {"constr_sec", constr_sec},
