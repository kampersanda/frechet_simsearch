...
struct Curve {
    ...
    Curve(size_t id) : id(id), points(std::vector<Point>()) {}
    Curve(size_t id, const std::vector<Point>& pts) : id(id), points(pts) {}
    ...
    inline Point front() const {
        return points.front();
    }

    inline Point back() const {
        return points.back();
    }
